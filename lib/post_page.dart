...
class _PostPageState extends State<PostPage> {
  ...
  void initState() {
    super.initState();
    _dropdownData = _getActivitiesAndGyms();
  }
  ...
  double? _progress;
  late Future<({List<String> activities, List<Map<String, dynamic>> gyms})> _dropdownData;

  /// Fetch all activities and gyms from db
  Future<({List<String> activities, List<Map<String, dynamic>> gyms})> _getActivitiesAndGyms() async {
    final allGyms = await helpers.getAllGymsWithProps(db.collection('gyms/budapest/gyms'));
    helpers.sortGymsByName(allGyms);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder(
        future: _dropdownData,
        builder: (BuildContext context, AsyncSnapshot<({List<String> activities, List<Map<String, dynamic>> gyms})> snapshot) {
          if (snapshot.hasData && snapshot.data != null && snapshot.connectionState == ConnectionState.done) {
            final snapData = snapshot.data as ({List<String> activities, List<Map<String, dynamic>> gyms});
            _dayTypes = snapData.activities;
            _gyms = snapData.gyms;
